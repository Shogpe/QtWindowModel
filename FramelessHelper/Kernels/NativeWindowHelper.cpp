...
NativeWindowHelper::NativeWindowHelper(QWindow *window, NativeWindowTester *tester) : QObject(window), d_ptr(new NativeWindowHelperPrivate()) {
    d_ptr->q_ptr = this;
...
NativeWindowHelper::NativeWindowHelper(QWindow *window) : QObject(window), d_ptr(new NativeWindowHelperPrivate()) {
    d_ptr->q_ptr = this;
...
bool NativeWindowHelper::nativeEventFilter(void *msg, long *result) {
    ...
    if (WM_NCHITTEST == lpMsg->message) {
        *result = d->hitTest(pos.x(), pos.y());
        ...
        lpMinMaxInfo->ptMinTrackSize.x = d->window->minimumWidth() * d->scaleFactor;
        lpMinMaxInfo->ptMinTrackSize.y = d->window->minimumHeight() * d->scaleFactor;
        lpMinMaxInfo->ptMaxTrackSize.x = d->window->maximumWidth() * d->scaleFactor;
        lpMinMaxInfo->ptMaxTrackSize.y = d->window->maximumHeight() * d->scaleFactor;
        if (result) *result = HTNOWHERE;
        return true;
...
NativeWindowHelperPrivate::NativeWindowHelperPrivate() : q_ptr(nullptr), window(nullptr), tester(nullptr), oldWindow(NULL), scaleFactor(1.0) {}

NativeWindowHelperPrivate::~NativeWindowHelperPrivate() {
    if (window) NativeWindowFilter::deliver(window, nullptr);
}
...
void NativeWindowHelperPrivate::updateWindowStyle() {
    ...
    if (QtWin::isCompositionEnabled()) newStyle |= WS_CAPTION;

    if (window->flags() & Qt::CustomizeWindowHint) {
        if (window->flags() & Qt::WindowSystemMenuHint) newStyle |= WS_SYSMENU;
        if (window->flags() & Qt::WindowMinimizeButtonHint) newStyle |= WS_MINIMIZEBOX;
        if (window->flags() & Qt::WindowMaximizeButtonHint) newStyle |= WS_MAXIMIZEBOX;
    } else {
    ...
    if (QtWin::isCompositionEnabled()) QtWin::extendFrameIntoClientArea(window, 1, 1, 1, 1);
}
...
int NativeWindowHelperPrivate::hitTest(int x, int y) const {
    ...
    if (top <= 0) top = GetSystemMetrics(SM_CYFRAME);
    if (left <= 0) left = GetSystemMetrics(SM_CXFRAME);
    if (right <= 0) right = GetSystemMetrics(SM_CXFRAME);
    if (bottom <= 0) bottom = GetSystemMetrics(SM_CYFRAME);

    auto pos = window->mapFromGlobal(pos_global);
    auto wfg = window->geometry();

    auto result =
        (Left * (pos.x() <= left)) | (Right * (pos.x() + right >= wfg.width())) | (Top * (pos.y() <= top)) | (Bottom * (pos.y() + bottom >= wfg.height()));
...
QRect NativeWindowHelperPrivate::availableGeometry() {
    ...
    auto hMonitor = MonitorFromWindow(hWnd, MONITOR_DEFAULTTONULL);
    if (!hMonitor || !GetMonitorInfoW(hMonitor, &mi)) {
        Q_ASSERT(NULL != hMonitor);
        return oldRect;
    }
    oldRect.setRect(mi.rcWork.left, mi.rcWork.top, mi.rcWork.right - mi.rcWork.left, mi.rcWork.bottom - mi.rcWork.top);
    return oldRect;
}
